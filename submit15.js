    ...
    $(document).on('click', '.guidem-button', function () {
        ...
        if (isBanned()) {
            ...
            clickedButton.prop('disabled', true)
                         .addClass('grayed-out-button'); // Disable and gray out the button when banned
            return;
        }
        ...
        if (incorrectAttempts > 5 && Date.now() - cooldownEndTime < 10000) {
            ...
            clickedButton.prop('disabled', true)
                         .addClass('grayed-out-button'); // Disable and gray out the button during cooldown
            setTimeout(function () {
                clickedButton.prop('disabled', false)
                             .removeClass('grayed-out-button'); // Re-enable and remove gray when cooldown ends
            }, 5000);
            return;
        }
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                ...
                clickedButton.prop('disabled', false)
                             .removeClass('grayed-out-button'); // Re-enable and remove gray on error
            }
        });
    });

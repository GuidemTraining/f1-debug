$(document).ready(function () {
    ...
    // Load ban and cooldown status from local storage
    var banEndTime = localStorage.getItem('banEndTime');
    var storedCooldownEndTime = localStorage.getItem('cooldownEndTime');
    if (banEndTime) {
        banEndTime = parseInt(banEndTime);
    }
    if (storedCooldownEndTime) {
        storedCooldownEndTime = parseInt(storedCooldownEndTime);
        if (storedCooldownEndTime > Date.now()) {
            cooldownEndTime = storedCooldownEndTime;
        }
    }
    ...
    function resetIncorrectAttempts() {
        incorrectAttempts = 0;
        banEndTime = 0;
        // Remove ban status from local storage
    ...
    function updateNextAllowedTryTime(minutes) {
        ...
        // Store ban status in local storage

document.addEventListener("DOMContentLoaded", function () {
    ...
    // Define the "Start Exam" button and check for the existence of CoursePlayerV2
    var examTimerBtn = document.getElementById("examTimerBtn");
    var coursePlayerExists = typeof CoursePlayerV2 !== 'undefined';

    // Define a flag to track whether the event listener is added
    var eventListenerAdded = false;

    // Add an initial event listener to the "Start Exam" button
    if (examTimerBtn && !eventListenerAdded) {
        examTimerBtn.addEventListener("click", startExam);
        eventListenerAdded = true;
    }

    // Check for CoursePlayerV2 and register the onContentChange event handler
    if (coursePlayerExists) {
    ...
    function onContentChange() {
        ...
        // Remove the existing event listener from the "Start Exam" button
        if (examTimerBtn && eventListenerAdded) {
            examTimerBtn.removeEventListener("click", startExam);
